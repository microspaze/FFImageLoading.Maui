...
#pragma warning restore RECS0108 // Warns about static fields in generic types
		...
		{
			if (Parameters.SourceType == ImageSourceType.Stream && Configuration.StreamChecksumsAsKeys && string.IsNullOrWhiteSpace(Parameters.CustomCacheKey))
			{
				...
				if (!_isLoadingPlaceholderLoaded && !string.IsNullOrWhiteSpace(Parameters.LoadingPlaceholderPath))
				{
					await ShowPlaceholder(Parameters.LoadingPlaceholderPath, KeyForLoadingPlaceholder,
										  Parameters.LoadingPlaceholderSourceType, true).ConfigureAwait(false);
				}
		...
		private void SetKeys()
		{
			...
			if (Parameters.SourceType == ImageSourceType.Stream)
			{
		...
		protected abstract Task<TDecoderContainer> TransformAsync(TDecoderContainer bitmap, IList<ITransformation> transformations, string path, ImageSourceType sourceType, bool isPlaceholder);
		...
		protected virtual async Task<TImageContainer> GenerateImageAsync(string path, ImageSourceType sourceType, Stream imageData, ImageInformation imageInformation, bool enableTransformations, bool isPlaceholder)
		{
			using (imageData)
			{
				...
				var decoderContainer = await decoder.DecodeAsync(
					imageData, path, sourceType, imageInformation, Parameters).ConfigureAwait(false);
				...
				if (enableTransformations && Parameters.Transformations != null && Parameters.Transformations.Count > 0)
				{
					...
					else
					{
						decoderContainer.Image = await TransformAsync(
							decoderContainer.Image, transformations, path, sourceType, isPlaceholder).ConfigureAwait(false);
					}
				}
		...
		protected virtual async Task<TImageContainer> GenerateImageAsync(string path, ImageSourceType sourceType, IDecodedImage<object> decoded, ImageInformation imageInformation, bool enableTransformations, bool isPlaceholder)
		{
			var decoderContainer = new DecodedImage<TDecoderContainer>()
			{
				...
				AnimatedImages = decoded.AnimatedImages?.Select(
					v => new AnimatedImage<TDecoderContainer> { Delay = v.Delay, Image = v.Image as TDecoderContainer })
										.ToArray() as IAnimatedImage<TDecoderContainer>[]
			};
			...
			if (enableTransformations && Parameters.Transformations != null && Parameters.Transformations.Count > 0)
			{
				...
				if (decoderContainer.IsAnimated && decoderContainer.AnimatedImages != null)
				{
					foreach (var animatedImage in decoderContainer.AnimatedImages)
					{
						animatedImage.Image = await TransformAsync(
							animatedImage.Image, transformations, path, sourceType, isPlaceholder).ConfigureAwait(false);
					}
				}
				else
				{
		...
		public virtual async Task<bool> TryLoadFromMemoryCacheAsync()
		{
			try
			{
				...
				if (result)
				{
					ImageService.Logger.Debug($"Image loaded from cache: {Key}");
					IsCompleted = true;
			...
			catch (Exception ex)
			{
				...
				if (ex is OperationCanceledException)
				{
					if (Configuration.VerboseLoadingCancelledLogging)
					{
						ImageService.Logger.Debug($"Image loading cancelled: {Key}");
					}
				}
				else
				{
		...
		protected virtual async Task ShowPlaceholder(string path, string key, ImageSourceType sourceType, bool isLoadingPlaceholder)
		{
			if (Parameters.Preload)
				return;
			...
			if (!await TryLoadFromMemoryCacheAsync(key, false, false, isLoadingPlaceholder).ConfigureAwait(false))
			{
				try
				{
					...
					var loadResolver = customResolver ?? ImageService.DataResolverFactory.GetResolver(path, sourceType, Parameters);
					...
					try
					{
						...
						{
							await using (loadImageData.Stream)
							{
								loadImage = await GenerateImageAsync(path, sourceType, loadImageData.Stream, loadImageData.ImageInformation, TransformPlaceholders, true).ConfigureAwait(false);
							}
						}
						else
						{
							loadImage = await GenerateImageAsync(path, sourceType, loadImageData.Decoded, loadImageData.ImageInformation, TransformPlaceholders, true).ConfigureAwait(false);
						}
		...
		public async Task RunAsync()
		{
			...
			try
			{
				...
				if (!(await TryLoadFromMemoryCacheAsync().ConfigureAwait(false)))
				{
					...
					ImageService.Logger.Debug($"Generating/retrieving image: {Key}");
					var resolver = Parameters.CustomDataResolver ?? ImageService.DataResolverFactory.GetResolver(Parameters.Path, Parameters.SourceType, Parameters);
					...
					if (Parameters.Preload && Parameters.CacheType.HasValue && Parameters.CacheType.Value == CacheType.Disk)
					{
						if (loadingResult == LoadingResult.Internet)
							ImageService.Logger?.Debug($"DownloadOnly success: {Key}");
					...
					{
						await using (imageData.Stream)
						{
							image = await GenerateImageAsync(Parameters.Path, Parameters.SourceType, imageData.Stream, imageData.ImageInformation, true, false).ConfigureAwait(false);
						}
					}
					else
					{
						image = await GenerateImageAsync(Parameters.Path, Parameters.SourceType, imageData.Decoded, imageData.ImageInformation, true, false).ConfigureAwait(false);
					}
					...
					try
					{
						...
						if (!Parameters.Preload && !string.IsNullOrWhiteSpace(Parameters.ErrorPlaceholderPath))
						{
							await ShowPlaceholder(Parameters.ErrorPlaceholderPath, KeyForErrorPlaceholder,
												  Parameters.ErrorPlaceholderSourceType, false).ConfigureAwait(false);
						}
					}
					catch (Exception ex2)
					{
						if (ex2 is not OperationCanceledException)
						{
							ImageService.Logger.Error($"Image loading failed: {Key}", ex);
						}
					}
