...
namespace FFImageLoading.Work
{
    public class WorkScheduler : IWorkScheduler
    {
        ...
        {
            PendingTasks.Enqueue(task, task.Parameters.Priority ?? GetDefaultPriority(task.Parameters.SourceType));
        }
        ...
        protected async Task TakeFromPendingTasksAndRunAsync()
        {
            ...
            lock (_lock)
            {
                ...
                {
                    urlTasksCount = RunningTasks.Count(v => v.Value != null && !v.Value.Parameters.Preload && v.Value.Parameters.SourceType == ImageSourceType.Url);
                ...
                while (tasksToRun.Count < numberOfTasks && PendingTasks.TryDequeue(out task))
                {
                    ...
                    {
                        if (!task.Parameters.Preload && (urlTasksCount == 0 || task.Parameters.SourceType != ImageSourceType.Url))
        ...
        protected virtual int GetDefaultPriority(ImageSourceType sourceType)
        {
            if (sourceType == ImageSourceType.ApplicationBundle || sourceType == ImageSourceType.CompiledResource)
                return (int)LoadingPriority.Normal + 2;

            if (sourceType == ImageSourceType.Filepath)
